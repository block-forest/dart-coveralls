...
void main() {
  var lcovTest = new File(p.join(p.current, 'test', 'lcov_test.txt')).readAsStringSync();

  group("LcovPart", () {
    test("parse", () {
      var document = LcovDocument.parse(lcovTest);
      expect(document.parts.length, equals(2));
      expect(document.parts.first.heading, equals("SF:collection/src/utils.dart"));
      expect(document.parts.first.fileName, equals("collection/src/utils.dart"));
      expect(document.parts.first.content, equals("DA:12,0\n"));
      expect(document.parts.last.heading, equals("SF:string_scanner/src/utils.dart"));
